...
const FIRMWARE_INO: &str = include_str!("../../firmware/zerospider-arduino/zerospider-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "zerospider-arduino";
...
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ...
    let temp_dir = std::env::temp_dir().join(format!("zerospider_flash_{}", uuid::Uuid::new_v4()));
