...
// in `~/.zerospider/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.zerospider/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(zerospider_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: zerospider_dir.join(".secret_key"),
            enabled,
        }
    }
